...
export default async function handler(req, res) {
  ...
  try {
    ...
    `);
    results.push('inventory_movements table created');
    ...
    `);
    results.push('incentive_policies table created');
    ...
    `);
    results.push('order_incentives table created');
    ...
    `);
    results.push('inquiries table created');
    ...
    // Sample data
    const empCheck = await query('SELECT COUNT(*) as count FROM employees');
    if (parseInt(empCheck.rows[0].count) === 0) {
      ...
      results.push('sample employees inserted');
      ...
      results.push('sample inventory inserted');
      ...
      results.push('sample customers inserted');
      ...
      results.push('sample transactions inserted');
      ...
      results.push('sample orders inserted');
    } else {
      results.push('sample data already exists');
    }
